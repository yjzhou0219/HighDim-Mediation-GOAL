...
for (j in 1:length(a1_all_20)){
  ...
  ## true mediation effects (assume no interaction)
  a1 <- a1_all_20[j]
  true_vals <- c(
    TE=a1*(yita[2]+yita[1]*yita[3]),
    NDE1=yita[2]*a1,
    NDE0=yita[2]*a1,
    NIE1=(a1-0)*yita[1]*yita[3],
    NIE0=(a1-0)*yita[1]*yita[3]
  )
  ...
  coverage_number <- as.data.frame(do.call(rbind,boot_results_a1[,3]))
  rownames(coverage_number) <- paste(rownames(coverage_number),a1_all_20_char[j],sep = "_")
  coverage_list[[j]] <- coverage_number
...
write.csv(ntrim_df_GOAL_S1,file = "S1_sost_sim_ntrim_GOAL.csv")

coverage_df <- as.data.frame(do.call(rbind,coverage_list))
coverage_df <- rbind(coverage_df,colMeans(coverage_df,na.rm = T))
