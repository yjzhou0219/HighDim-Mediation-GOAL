...
mediation.cont<-function(y,a,m,x_ax,x_amx,a0,a1,ATET=FALSE,trim=0.05,lognorm=FALSE,bw){ 
...
bootstrap.mediation.cont<-function(y,a,m,x_ax,x_amx,a0,a1,ATET=FALSE,trim=0.05,lognorm=FALSE,bw,boot=1999,cluster=NULL){ 
...
medweightcont<-function(y,a,m,x_ax,x_amx,a0,a1,ATET=FALSE,trim=0.05,lognorm=FALSE,bw=NULL,boot=1999,cluster=NULL){
  ...
  point_est=temp[1:(length(temp)-1)] 
  
  
  boot_dist=bootstrap.mediation.cont(y=y,a=a,m=m,x_ax=x_ax,x_amx=x_amx,a0=a0,a1=a1,ATET=ATET,trim=trim,lognorm=lognorm,bw=bw,boot=boot,cluster=cluster)
  ## bootstrap 95% CI
  ci_lower <- apply(boot_dist[,1:(ncol(boot_dist)-1)], 2, quantile, probs = 0.025, na.rm = TRUE)
  ci_upper <- apply(boot_dist[,1:(ncol(boot_dist)-1)], 2, quantile, probs = 0.975, na.rm = TRUE)
  
  se=apply(boot_dist[,1:(ncol(boot_dist)-1)], 2, sd) 
  
  p_val=2*pnorm(-abs(point_est/se)) 
  ...
  if (ATET==FALSE) colnames(results_summary)=c("ATE", "dir.treat", "dir.control", "indir.treat", "indir.control")
  if (ATET==TRUE) colnames(results_summary)=c("ATET", "dir.treat", "dir.control", "indir.treat", "indir.control")
